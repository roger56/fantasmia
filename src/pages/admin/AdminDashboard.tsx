...
import { Users, Building2, Grid3x3, FileText, Settings, BookOpen, Calendar, Sparkles, Link } from "lucide-react";
...
const AdminDashboard = () => {
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-1">
                <h3 className="text-lg font-semibold text-emerald-900 truncate">{card.title}</h3>
                {card.badge && (
  ...
  return (
    <AdminLayout title="Dashboard Amministratore" subtitle="Console di gestione sistema Fantasmia">
      <div className="space-y-8">
        ...
        <section>
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">{governanceCards.map(renderCard)}</div>
        </section>
        ...
        <section>
          ...
          <p className="text-sm text-emerald-600 mb-4">
            Queste sono copie indipendenti degli strumenti SU, modificabili per ora senza impatti sulla webapp.
          </p>
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">{toolsCards.map(renderCard)}</div>
        </section>
      </div>
    </AdminLayout>
  );
