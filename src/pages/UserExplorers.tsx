...
const UserExplorers = () => {
  ...
  const loadStories = async () => {
    try {
      ...
      const explorerStories = allStories
        .filter(story => story.category === 'explorers')
        .map(story => ({
          ...story,
          language: story.language || 'italian'
        }));
      setStories(explorerStories);
    } catch (error) {
      console.error('Error loading explorer stories:', error);
      toast({
        title: "Errore",
        description: "Errore nel caricamento delle storie degli esploratori",
        variant: "destructive"
      });
    }
  };

  const handleRead = (story: ExplorerStory) => {
  ...
  const handleImageClick = (story: ExplorerStory) => {
    if (story.has_image) {
      toast({
  ...
  const handleBooksClick = (story: ExplorerStory) => {
    setSelectedStory(story);
    setShowBooksDialog(true);
  };

  const handleStoryClick = (story: ExplorerStory) => {
  ...
  return (
    <StoryLayout
      ...
      subtitle="Storie di avventurieri e scopritori"
              ...
              <div className="space-y-2">
                {stories.length === 0 ? (
                  <div className="text-center text-gray-500 py-8">
                    Nessuna storia di esploratori disponibile
                  </div>
                ) : (
                  stories.map((story) => (
                    <div 
