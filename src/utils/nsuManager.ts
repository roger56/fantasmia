...
export const createNSU = async (suId: string, payload: CreateNSUPayload): Promise<NSUCreationResult> => {
  await fantasMiaDB.init();
  ...
  const duplicate = existingProfiles.find((p) => p.name.toLowerCase() === payload.name.toLowerCase());
  if (duplicate) {
...
export const authenticateNSU = async (profileId: string, password: string): Promise<AuthResult> => {
  await fantasMiaDB.init();
...
export const resetNSUPassword = async (profileId: string, newPassword?: string): Promise<string> => {
  await fantasMiaDB.init();
  const allProfiles = await fantasMiaDB.getProfiles();
  const profile = allProfiles.find((p) => p.id === profileId);
...
export const updateNSUPassword = async (profileId: string, newPassword: string): Promise<void> => {
